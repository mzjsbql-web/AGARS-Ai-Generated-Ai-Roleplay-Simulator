...
class ProjectManager:
    ...
    @classmethod
    def _get_entity_database_path(cls, project_id: str) -> str:
        """获取预提取实体数据库的存储路径"""
        return os.path.join(cls._get_project_dir(project_id), 'entity_database.json')
    ...
    @classmethod
    def save_entity_database(cls, project_id: str, entity_database: Dict[str, Any]) -> None:
        """保存预提取的实体数据库"""
        path = cls._get_entity_database_path(project_id)
        with open(path, 'w', encoding='utf-8') as f:
            json.dump(entity_database, f, ensure_ascii=False, indent=2)

    @classmethod
    def get_entity_database(cls, project_id: str) -> Optional[Dict[str, Any]]:
        """读取预提取的实体数据库，不存在则返回 None"""
        path = cls._get_entity_database_path(project_id)
