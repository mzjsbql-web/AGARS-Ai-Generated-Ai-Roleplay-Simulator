...
logger = get_logger('agars.api.simulation')
    ...
    请求（JSON）：
        {
            ...
            "graph_id": "agars_xxxx",    // 可选，如不提供则从project获取
...
def delete_simulation(simulation_id: str):
    ...
    try:
        ...
        return jsonify({
            "success": True,
            "message": f"已删除模拟记录: {simulation_id}"
        })

    except Exception as e:
        logger.error(f"删除模拟记录失败: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e)
        }), 500
    ...
    try:
        ...
        for sim in simulations:
            ...
            if config:
                ...
                sim_dict["custom_title"] = config.get("custom_title", "")
            ...
            # 跳过 narrative 模式的 simulation 记录（叙事历史由 /narrative/history 单独提供）
            if sim_dict.get("mode") == "narrative":
                continue

            # 添加记录类型和版本号
            sim_dict["record_type"] = "simulation"
...
@simulation_bp.route('/<simulation_id>/rename', methods=['PATCH'])
def rename_simulation(simulation_id: str):
    ...
    try:
        ...
        if not os.path.isfile(config_path):
            return jsonify({"success": False, "error": "模拟配置不存在"}), 404
