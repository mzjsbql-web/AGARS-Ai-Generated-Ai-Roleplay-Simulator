...
logger = get_logger('agars.report_agent')
...
【重要】需要OASIS模拟环境正在运行才能使用此功能！""",
    ...
    def plan_outline(
        ...
        # 构建规划prompt
        system_prompt = get_system('report_outline')

        user_prompt = safe_render(get_template('report_outline'), {
            'simulation_requirement': self.simulation_requirement,
            'total_nodes': str(context.get('graph_statistics', {}).get('total_nodes', 0)),
            'total_edges': str(context.get('graph_statistics', {}).get('total_edges', 0)),
            'entity_types_list': str(list(context.get('graph_statistics', {}).get('entity_types', {}).keys())),
            'active_agent_count': str(context.get('total_entities', 0)),
            'sample_facts_json': json.dumps(context.get('related_facts', [])[:10], ensure_ascii=False, indent=2),
        })
    ...
    def _generate_section_react(
        ...
        system_prompt = f"""{get_system('report_section')}
...
═══════════════════════════════════════════════════════════════"""
    ...
    def chat(
        ...
        # 构建系统提示
        system_prompt = f"""{get_system('report_chat')}
