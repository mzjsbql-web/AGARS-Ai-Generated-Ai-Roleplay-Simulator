...
from .prompt_config import get_system, get_template, safe_render

logger = get_logger('agars.oasis_profile')
...
class OasisProfileGenerator:
    ...
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        ...
        try:
            ...
            # 处理边搜索结果，只保留明确提及目标实体名称的事实（语义搜索过滤）
            all_facts = set()
            entity_name_lower = entity_name.lower()
            if edge_result and hasattr(edge_result, 'edges') and edge_result.edges:
                for edge in edge_result.edges:
                    if hasattr(edge, 'fact') and edge.fact:
                        if entity_name_lower in edge.fact.lower():
                            all_facts.add(edge.fact)

            # 补充直连边（结构性检索）：source_node_uuid 或 target_node_uuid == entity.uuid
            # 这些边无需名称过滤，UUID 匹配即可保证与该实体直接相关
            for raw_edge in self._get_zep_all_edges():
                src = getattr(raw_edge, 'source_node_uuid', '') or ''
                tgt = getattr(raw_edge, 'target_node_uuid', '') or ''
                if src == entity.uuid or tgt == entity.uuid:
    ...
    def _generate_profile_with_llm(
        ...
        for attempt in range(max_attempts):
            try:
                ...
                _mon_start = time.time()
                response = self.client.chat.completions.create(
                    model=self.model_name,
                    messages=_mon_msgs,
                    response_format={"type": "json_object"},
                    temperature=0.7 - (attempt * 0.1),  # 每次重试降低温度
                    timeout=120.0,  # 单次调用 120 秒超时
                )

                content = response.choices[0].message.content
                _mon_dur = (time.time() - _mon_start) * 1000
                _usage = response.usage
                _mon_tok = {"prompt": _usage.prompt_tokens, "completion": _usage.completion_tokens, "total": _usage.total_tokens} if _usage else None
                monitor.log_full(source="OasisProfileGenerator", model=self.model_name, messages=_mon_msgs, response=content, duration_ms=_mon_dur, tokens=_mon_tok)
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        """获取系统提示词"""
        if is_individual:
            return get_system('oasis_profile')
        else:
            return get_system('oasis_profile_group')
    ...
    def _build_individual_persona_prompt(
        ...
        entity_attributes_json = json.dumps(entity_attributes, ensure_ascii=False) if entity_attributes else "无"
        context_str = context[:3000] if context else "无额外上下文"

        return safe_render(get_template('oasis_profile'), {
            'entity_name': entity_name,
            'entity_type': entity_type,
            'entity_summary': entity_summary,
            'entity_attributes_json': entity_attributes_json,
    ...
    def _build_group_persona_prompt(
        ...
        return safe_render(get_template('oasis_profile_group'), {
            'entity_name': entity_name,
            'entity_type': entity_type,
            'entity_summary': entity_summary,
            'entity_attributes_json': entity_attributes_json,
    ...
    def generate_profiles_from_entities(
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=parallel_count) as executor:
            ...
            # 收集结果（每个 future 设置 180 秒超时，避免无限等待）
            for future in concurrent.futures.as_completed(future_to_entity):
                idx, entity = future_to_entity[future]
                entity_type = entity.get_entity_type() or "Entity"

                try:
                    result_idx, profile, error = future.result(timeout=180)
