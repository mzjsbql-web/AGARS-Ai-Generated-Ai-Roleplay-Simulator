...
const renderGraph = () => {
  ...
  // Store node ref for external highlight
...
// 监听 highlightNodeId 变化，高亮对应节点并居中
watch(() => props.highlightNodeId, (nodeId) => {
  if (!nodeSelectionRef || !nodeId) {
    // 清除所有高亮
    if (nodeSelectionRef) {
      nodeSelectionRef.classed('node-highlighted', false)
  ...
  // 查找匹配节点
  nodeSelectionRef.each(function (d) {
    if (d.rawData?.uuid === nodeId || d.id === nodeId) {
      const el = d3.select(this)
      el.classed('node-highlighted', true)
        .attr('stroke', '#FF5722').attr('stroke-width', 4)

      // Zoom to center on the node
      if (svgZoomRef && svgSelectionRef && graphContainer.value) {
        const w = graphContainer.value.clientWidth
        const h = graphContainer.value.clientHeight
        const transform = d3.zoomIdentity.translate(w / 2 - d.x, h / 2 - d.y)
        svgSelectionRef.transition().duration(500).call(svgZoomRef.transform, transform)
      }
    }
  })
})
...
/* Highlighted node pulse */
:deep(.node-highlighted) {
  animation: pulse-ring 1s infinite;
}

@keyframes pulse-ring {
  0% { filter: drop-shadow(0 0 2px rgba(255, 87, 34, 0.4)); }
  50% { filter: drop-shadow(0 0 8px rgba(255, 87, 34, 0.8)); }
  100% { filter: drop-shadow(0 0 2px rgba(255, 87, 34, 0.4)); }
}
