<template>
  <div class="process-page">
    <!-- 顶部导航栏 -->
    <nav class="navbar">
      <div class="nav-brand" @click="goHome"><span style="color:#FF4500">A</span>GARS</div>
        ...
        <div class="process-content">
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
            ...
            <div class="phase-detail">
              ...
              <!-- 等待本体完成 -->
              <div class="detail-section waiting-state" v-if="currentPhase < 0 || (currentPhase === 0 && !projectData?.ontology)">
                <div class="waiting-hint">等待本体生成完成...</div>
              </div>

              <!-- 构建参数设置 -->
              <div class="detail-section build-settings" v-if="projectData?.ontology">
                <div class="detail-label">构建参数</div>
                <div class="settings-grid">
                  <div class="setting-item">
                    <label class="setting-label">文本分块大小 (chunk_size)</label>
                    <input
                      type="number"
                      v-model.number="buildSettings.chunk_size"
                  ...
                  <div class="setting-item">
                    <label class="setting-label">每批处理块数 (batch_size)</label>
                    <input
                      type="number"
                      v-model.number="buildSettings.batch_size"
                      :disabled="currentPhase >= 1"
                      min="1"
                      max="50"
                      step="1"
                  ...
                  <div class="setting-item">
                    <label class="setting-label">分块重叠大小 (chunk_overlap)</label>
                    <input
                      type="number"
                      v-model.number="buildSettings.chunk_overlap"
                      :disabled="currentPhase >= 1"
                      min="0"
                      max="500"
                      step="10"
                ...
                <div class="settings-note">
                  较大的 chunk_size 和 batch_size 可加快构建速度，但消耗更多 Token
                </div>

                <!-- 开始构建按钮 -->
                <button
                  class="build-start-btn"
          ...
          <!-- 手动合并节点 -->
          <div class="process-phase" v-if="graphData?.nodes?.length > 0">
            <div class="phase-header" @click="showMergePanel = !showMergePanel" style="cursor:pointer">
              <span class="phase-num">⇄</span>
              <div class="phase-info">
                <div class="phase-title">手动合并节点</div>
                <div class="phase-api">POST /api/graph/merge-entities</div>
              </div>
              <span class="phase-status" style="font-size:11px;color:#999">{{ showMergePanel ? '收起' : '展开' }}</span>
            </div>

            <div v-if="showMergePanel" class="phase-detail">
              <div class="detail-section">
                <div class="detail-content" style="font-size:12px;color:#666;margin-bottom:10px">
                  点击节点可选中/取消，选中 2 个及以上后点击合并按钮。边数最多的节点将作为主节点。
                </div>

                <!-- 已选 -->
                <div v-if="mergeSelected.size > 0" style="margin-bottom:10px">
                  <div style="font-size:10px;color:#AAA;margin-bottom:6px">已选 {{ mergeSelected.size }} 个</div>
                  <div style="display:flex;flex-wrap:wrap;gap:6px;margin-bottom:8px">
                    <span
                      v-for="uuid in mergeSelected"
                      :key="uuid"
                      @click="mergeSelected.delete(uuid)"
                      style="background:#FF5722;color:#FFF;font-size:11px;padding:3px 8px;border-radius:12px;cursor:pointer;font-family:'JetBrains Mono',monospace"
                    >{{ mergeNodeMap[uuid]?.name || uuid.slice(0,8) }} ×</span>
                  </div>
                  <button class="build-start-btn" :disabled="mergeSelected.size < 2 || isMerging" @click="handleMerge" style="margin-top:0">
                    <span v-if="isMerging" class="building-spinner" style="width:12px;height:12px;border-width:2px"></span>
                    {{ isMerging ? '合并中...' : `合并所选 ${mergeSelected.size} 个节点` }}
                  </button>
                  <div v-if="mergeMsg" :style="{marginTop:'8px',fontSize:'11px',padding:'6px 10px',borderRadius:'4px',background: mergeOk?'#E8F5E9':'#FFEBEE',color:mergeOk?'#2E7D32':'#C62828'}">
                    {{ mergeMsg }}
                  </div>
                </div>

                <!-- 搜索 -->
                <input v-model="mergeSearch" placeholder="搜索节点名称..." class="setting-input" style="width:100%;margin-bottom:8px" />

                <!-- 节点列表 -->
                <div style="max-height:220px;overflow-y:auto;border:1px solid #EAEAEA;border-radius:4px">
                  <div
                    v-for="node in filteredMergeNodes"
                    :key="node.uuid"
                    @click="toggleMergeSelect(node.uuid)"
                    :style="{
                      display:'flex',alignItems:'flex-start',gap:'10px',padding:'8px 10px',
                      cursor:'pointer',borderBottom:'1px solid #F5F5F5',
                      background: mergeSelected.has(node.uuid) ? '#FFF3E0' : 'transparent'
                    }"
                  >
                    <span style="font-size:14px;color:#FF5722;flex-shrink:0;margin-top:1px">
                      {{ mergeSelected.has(node.uuid) ? '☑' : '☐' }}
                    </span>
                    <div style="display:flex;flex-direction:column;gap:2px;min-width:0">
                      <span style="font-size:12px;font-weight:600;font-family:'JetBrains Mono',monospace">{{ node.name }}</span>
                      <span style="font-size:11px;color:#888;line-height:1.4;word-break:break-all">{{ node.summary?.slice(0,80) || '—' }}</span>
                    </div>
                  </div>
                </div>
              </div>
...
import { generateOntology, getProject, buildGraph, getTaskStatus, getGraphData, mergeEntities } from '../api/graph'
...
const mergeSelected = reactive(new Set())
const isMerging = ref(false)
const mergeMsg = ref('')
const mergeOk = ref(true)

const mergeNodeMap = computed(() => {
  const map = {}
  for (const n of (graphData.value?.nodes || [])) map[n.uuid] = n
  return map
})
...
const toggleMergeSelect = (uuid) => {
  if (mergeSelected.has(uuid)) mergeSelected.delete(uuid)
  else mergeSelected.add(uuid)
}

const handleMerge = async () => {
  if (mergeSelected.size < 2 || isMerging.value) return
  const graphId = projectData.value?.graph_id
  if (!graphId) { mergeMsg.value = '缺少 graph_id'; mergeOk.value = false; return }

  isMerging.value = true
  mergeMsg.value = ''
  try {
    const res = await mergeEntities(graphId, [...mergeSelected])
    if (res.success) {
      mergeOk.value = true
      mergeMsg.value = `合并成功，保留节点: ${res.data?.canonical_uuid?.slice(0, 8)}...`
      mergeSelected.clear()
      await fetchGraphData()
    } else {
      mergeOk.value = false
      mergeMsg.value = res.error || '合并失败'
    }
  } catch (e) {
    mergeOk.value = false
    mergeMsg.value = e.message
  } finally {
...
const statusText = computed(() => {
  ...
  if (currentPhase.value === 0 && projectData.value?.ontology) return '待构建图谱'
...
const loadProject = async () => {
  try {
    ...
    if (response.success) {
      ...
      // 从项目数据初始化构建参数
      if (response.data.chunk_size) buildSettings.value.chunk_size = response.data.chunk_size
      if (response.data.batch_size) buildSettings.value.batch_size = response.data.batch_size
      if (response.data.chunk_overlap !== undefined) buildSettings.value.chunk_overlap = response.data.chunk_overlap
...
.build-start-btn:hover {
  background: #FF6B35;
}

.build-start-btn .btn-arrow {
  font-size: 1rem;
  transition: transform 0.2s;
}

.build-start-btn:hover .btn-arrow {
