...
const canSave = computed(() => projectData.value !== null && currentProjectId.value !== 'new')
...
const handleSave = async () => {
  ...
  const shortId = currentProjectId.value?.slice(0, 8) ?? ''
  saveNotification.value = `项目已保存（ID: ${shortId}…）· 可在主页历史记录中找到`
  addLog(`存档成功：项目 ${currentProjectId.value}`)
...
const statusText = computed(() => {
  ...
  if (currentPhase.value === 0 && projectData.value?.ontology) return 'Ready to Build'
  if (currentPhase.value === 0) return 'Generating Ontology'
  return 'Initializing'
})

const onUpdateSettings = (newSettings) => {
  Object.assign(buildSettings.value, newSettings)
}
...
const handleNewProject = async () => {
  ...
  try {
    ...
    if (res.success) {
      ...
      currentPhase.value = 0 // 停在 phase 0，等用户手动构建
    } else {
...
const loadProject = async () => {
  try {
    ...
    if (res.success) {
      ...
      // 从项目数据初始化构建参数
      if (res.data.chunk_size) buildSettings.value.chunk_size = res.data.chunk_size
      if (res.data.batch_size) buildSettings.value.batch_size = res.data.batch_size
      if (res.data.chunk_overlap !== undefined) buildSettings.value.chunk_overlap = res.data.chunk_overlap
